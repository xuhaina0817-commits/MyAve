...
// 从 Vite 环境变量中读取 API Key
// Vercel/Vite 要求 Key 必须以 VITE_ 开头才能在客户端访问
 ...
 * @param historyMessages - Optional array of past messages to restore.
 * @returns A promise resolving to an initialization message.
 */
export const initializeCharacterChat = async (characterId: string = 'mutsumi', historyMessages: Message[] = []): Promise<string> => {
    ...
    // Set System Prompt
    systemPrompt = character.systemInstruction;

    // Initialize History
    // FIX: Ensure correct mapping for all Message senders to ChatMessage roles
    conversationHistory = historyMessages.map(m => {
        let role: ChatMessage['role'];
        if (m.sender === Sender.USER) {
            role = 'user';
        } else if (m.sender === Sender.CHARACTER || m.sender === Sender.MUTSUMI) {
            role = 'assistant';
        } else {
            // Treat other senders (like SYSTEM) as system messages or ignore
            role = 'system'; 
        }
    ...
    }).filter(m => m.role !== 'system'); // Filter out system messages that don't belong in conversation flow
 ...
 * @param memberIds - Array of character IDs in the group.
 * @param historyMessages - Optional array of past messages to restore.
 * @returns A promise resolving to an initialization message.
 */
export const initializeGroupChat = async (memberIds: string[], historyMessages: Message[] = []): Promise<string> => {
    ...
    // 1. Build Group System Instruction (Logic looks good)
    ...
    systemPrompt = groupInstruction;

    // 2. Convert History with [CharacterID] prefixes for group context
    // FIX: Ensure correct mapping for all Message senders to ChatMessage roles
    conversationHistory = historyMessages.map(m => {
        let content = m.text;
        let role: ChatMessage['role'];
        ...
        if (m.sender === Sender.USER) {
            ...
            role = 'assistant';
            // Group chat history needs the [ID]: prefix for the model to understand who spoke
            if (currentSessionType === 'group' && m.characterId) {
                content = `[${m.characterId}]: ${m.text}`;
            }
        } else {
            role = 'system';
        }
 ...
 * * NOTE: For the frontend to work, you MUST export `sendMessageToMutsumi` as an alias
 * to satisfy the original component's import requirements, if you cannot change App.tsx.
 */
export const sendMessage = async (userMessage: string): Promise<Message[]> => {
    
    // Check for API Key presence first
    if (!API_KEY) {
        console.error("DeepSeek API Key is missing. Check Vercel Environment Variables (VITE_DEEPSEEK_API_KEY).");
        return [{
            id: Date.now().toString(),
            text: "系统错误：API 密钥未配置。请检查 Vercel 环境变量。",
            sender: Sender.SYSTEM, // Use SYSTEM sender for config errors
            timestamp: new Date(),
            characterId: currentMembers[0] || 'mutsumi'
        }];
    }
    ...
    try {
        // Prepare messages array for DeepSeek (System Instruction first, then History)
        ...
        const response = await fetch(API_URL, {
            method: 'POST',
            // NOTE: The Authorization header is now using the correctly read API_KEY
            headers: {
        ...
        if (!response.ok) {
            ...
            console.error("DeepSeek API Response Error:", errorData);
            throw new Error(`API Error ${response.status}: ${errorData.message || response.statusText}`);
        }

        const data = await response.json();
        const rawText = data.choices?.[0]?.message?.content || "……(没有回复)";
        ...
        if (currentSessionType === 'group') {
            ...
            // FIX: Modified regex to correctly capture the end of the content field, 
            // including potential multiple lines within one character's block
            const regex = /\[(\w+)]:\s*([\s\S]*?)(?=\n\[\w+]:|$)/g;
            let match;
            let found = false;
            let lastIndex = 0;
            ...
            while ((match = regex.exec(rawText)) !== null) {
                ...
                if (CHARACTERS[charId]) {
                    responses.push({
                        ...
                        timestamp: new Date(baseTime + responses.length * 50), // Slight delay for chat effect
                        characterId: charId
                    });
                }
            }

            // Fallback for messy output or single response in group chat context
            if (!found && rawText.trim()) {
                // Try to infer a single speaker from current members
                const defaultCharId = currentMembers[0] || 'mutsumi'; 
                responses.push({
        ...
        return [{
            id: Date.now().toString(),
            text: `……(信号中断。错误: ${error.message || '未知错误'})`,
            sender: Sender.CHARACTER,
            timestamp: new Date(),
            characterId: currentMembers[0] || 'mutsumi'
        }];
...
// --- Compatibility Exports for App.tsx ---

// FIX: To avoid modifying App.tsx, export the functions it expects:
// App.tsx expects: initializeMutsumiChat, sendMessageToMutsumi, resetChatSession, restoreChatSession
export const initializeMutsumiChat = async () => {
    // Default to Mutsumi chat initialization
    return initializeCharacterChat('mutsumi', []);
};

export const sendMessageToMutsumi = async (text: string): Promise<Message> => {
    // Call the unified sendMessage and return the first response (Mutsumi's)
...
// FIX: The original restoreChatSession seems to be handled by the historyMessages parameter 
// in the initialize functions. Let's create an export to satisfy the dependency.
export const restoreChatSession = async (messages: Message[], characterId: string = 'mutsumi') => {
    // Assuming this is used to re-initialize an existing single chat session
    await initializeCharacterChat(characterId, messages);
